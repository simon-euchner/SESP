...
sesp_coo *sesp_den_tocoo(sesp_den *A) {
    ...
    INT nnz = sesp_den_tocoo_worker(A, B);
    sesp_den_free(A);
    if (nnz) {
        B->rowis = realloc(B->rowis, sizeof(INT)*nnz);
        B->colis = realloc(B->colis, sizeof(INT)*nnz);
        B->data = realloc(B->data, sizeof(DTYPE)*nnz);
    } else {
        B->rowis = NULL;
        B->colis = NULL;
        B->data = NULL;
...
int sesp_den_tocoo_worker(const sesp_den *A, sesp_coo *B) {
    ...
    for (i=0; i<A->nrow; i++) {
        for (j=0; j<A->ncol; j++) {
            k = i*A->ncol;
            if (cabsl((DTYPE_M)A->data[k+j]) > SMALL) {
                B->rowis[nnz] = i;
                B->colis[nnz] = j;
